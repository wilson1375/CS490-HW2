...
class Net(nn.Module):
    def __init__(self):
        ...
        # L10
        self.conv10 = nn.Conv2d(in_channels=10, out_channels=10, kernel_size=1, stride=1)
        ...
        self.fc1 = nn.Linear(in_features=10, out_features=4096)
        self.fc2 = nn.Linear(in_features=4096,out_features=4096)
        self.fc3 = nn.Linear(in_features=4096,out_features=CLASSES)

    def forward(self, x):
        x = F.relu(self.conv1(x))
        x = self.pool1(F.relu(self.local_response1(x)))
        x = F.relu(self.conv2(x))
        x = self.pool2(F.relu(self.local_response2(x)))
        x = self.dropout3(F.relu(self.pool3(F.relu(self.conv3(x)))))
        x = F.relu(self.conv4(x))
        x = self.dropout5(F.relu(self.pool5(F.relu(self.conv5(x)))))
...
BATCH_SIZE=16
train_dataset=datasets.FashionMNIST('./data_fashionMNIST/',train=True,download=True,transform=transform_conf,)
test_dataset=datasets.FashionMNIST('./data_fashionMNIST/',train=False,download=True,transform=transform_conf)
